...
public class UISpawn : MonoBehaviour {
    ...
    private patientInfo readXML(string ID)
    {
        ...
        while (reader.ReadToFollowing("Patient"))
        {
            ...
            {
                if(innerReader.ReadContentAsString() == ID)
                {
                    innerReader.ReadToFollowing("Name");
                    patient.NAME = innerReader.ReadContentAsString();
                    innerReader.ReadToFollowing("Age");
                    patient.AGE = innerReader.ReadContentAsString();
                    innerReader.ReadToFollowing("DOB");
                    patient.DOB = innerReader.ReadContentAsString();
                    innerReader.ReadToFollowing("Sex");
                    patient.SEX = innerReader.ReadContentAsString();
                    innerReader.ReadToFollowing("Room");
                    patient.ROOM = innerReader.ReadContentAsString();
                    return patient;
                }
                else
                {
